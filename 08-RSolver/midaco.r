...
library(foreach)
library(doParallel) # change doMC to doParallel to support both Linux and Windows 
registerDoParallel(P) # change registerDoMC to registerDoParallel
...
while( istop == 0) 
{
      ...
      loopoutput <- foreach(c=1:P,  .combine='c', .export = 'problem_function') %dopar% # import "problem_function", otherwise, Windows will fail to load it into the worker. This parameter will be ignored on Linux
      {
