setwd(file.path(Sys.getenv("USERPROFILE"), "onedrive/app/r/r-play/08-rsolver"))
########################################################################
#
...
problem_function <- function(f, g, x) {
    ...
    g[1] = 62 * x[1] - 111 * x[1] ^ 2 - 83 * x[1] * x[2] + 111 * x[1] ^ 2 * x[2] - 7
...
############################# 
option.maxeval = 1e6 # Maximum evaluation budget (e.g. 1000000)
option.maxtime = 60 * 60 * 24 # Maximum time limit in Seconds (e.g. 1 Day = 60*60*24) 

# STEP 2.B: Printing options  
############################  
option.printeval = 1e4 # Print-Frequency for current best solution (e.g. 1000)
...
option.parallel = 8 # Serial: 0 or 1, Parallel: 2,3,4,5,6,7,8...
